...
type ColorTypeSelectProps = {
  value: ColorType;
  onChange: (v: ColorType) => void;
}

export default function ColorTypeSelect (props: ColorTypeSelectProps) {
  ...
  return (
    <div className="rcs-panel-type-select" onClick={handleTrigger} ref={optionsRef}>
        ...
        <div className="rcs-panel-type-select-options">
          {
            ColorTypeOptions.map(option => (
              <span
                ...
                onClick={() => { changeType(option.value as ColorType) }}
              >
                {option.label}
              </span>
            ))
